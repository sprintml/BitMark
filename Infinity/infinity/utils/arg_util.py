...
class Args(Tap):
    ...
    tau: float = 0.001                  # tau of self attention in GPT
    ...
    cfg: float = 10                    # >0: classifier-free guidance, drop cond with prob cfg
    ...
    fp16: int = 1                       # 1: fp16, 2: bf16, >2: fp16's max scaling multiplier todo: 记得让quantize相关的feature都强制fp32！另外residueal最好也是fp32（根据flash-attention）nn.Conv2d有一个参数是use_float16？
    ...
    # data
    pn: str = '1M'                        # pixel nums, choose from 0.06M, 0.25M, 1M
