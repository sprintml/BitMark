...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        summed_codes = 0
        if decode_per_scale:
            img_list, img_residual_list, img_residual_upscaled_list = [], [], []
        get_stat_data = True
        if get_stat_data:
            ret = {}
            for i in range(B):
            ...
            ret = {}
        last_index = 0 # Used for N-bit pattern
        token_size = self.codebook_dim 
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            logger.info(f"current stage: {si}, pn:{pn}" )
            cfg = cfg_list[si]
            if si >= trunk_scale:
                break
            ...
            # Add level embedding for block to feature (last stage), apply layer and sometimes cfg to feature
            for block_idx, b in enumerate(self.block_chunks):
                # last_stage shape: [4, 1, 2048], cond_BD_or_gss.shape: [4, 1, 6, 2048], ca_kv[0].shape: [64, 2048], ca_kv[1].shape [5], ca_kv[2]: int
                if self.add_lvl_embeding_only_first_block and block_idx == 0:
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad=need_to_pad) # Last stage is SOS
                if not self.add_lvl_embeding_only_first_block: # Adds same embedding in both cases
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad=need_to_pad)
                
                # Projection, add CFG etc
                for m in b.module:
                    ...
                    layer_idx += 1
            if (cfg != 1) and add_cfg_on_logits: # cfg defined in cfg list per residual, is true by defaul
            ...
            if self.use_bit_label:
                ...
                batch_size, seq_len, vocab_size = logits_BlV.shape
                logger.info(f"bs={batch_size}, seq_len={seq_len}, vocab_size={vocab_size}")
                idx_Bld = torch.empty((batch_size, seq_len), dtype=torch.int64, device="cuda")
                start = time.time()
                
                num_tokens = int(seq_len/token_size)
                if scales_injector and (si in scales_injector.inject_scales and scales_injector.teacher_forcing == False): 
                #if scales_injector and (si in scales_injector.inject_scales and scales_injector.teacher_forcing == False) or (si not in scales_injector.inject_scales and scales_injector.teacher_forcing): 
                    idx_Bld = scales_injector.inject_scales_bits[si]
                else:
                    if si in watermark.scales:
                        ### WATERMARKING START NOTE refactor this code into watermark inference object? 
                        logger.info(f"Applying watermarking on scale {si}")
                        match watermark.method:
                            case '2-bit_pattern':
                                if self.apply_spatial_patchify:
                                    input_shape = (1, pn[1], pn[2],token_size)  # NCHW
                                    input_tensor = torch.arange(token_size*pn[1]*pn[2]).reshape(input_shape).to('cuda')
                                    input_tensor= input_tensor.permute(0,3,1,2) # [B, 4d, h, w]
                                    input_tensor = torch.nn.functional.pixel_shuffle(input_tensor, 2) # [B, d, 2h, 2w]
                                    input_tensor = input_tensor.permute(0,2,3,1) # [B, 2h, 2w, d]
                                    bit_order = input_tensor.flatten()
                                else:
                                    bit_order = torch.arange(token_size*pn[1]*pn[2]).to('cuda')
                                for n in range(token_size):
                                    t_n = torch.arange(0, num_tokens, device="cuda")*token_size + n # Index of bit n in each token
                            ...
                            case 'N-bit_pattern':
                                processed_logits = watermark.logits_processor(watermark.message[...,last_index:seq_len+last_index], logits_BlV) 
                                last_index+=seq_len
                                idx_Bld= sample_with_top_k_top_p_also_inplace_modifying_logits_(processed_logits, rng=rng, top_k=top_k, top_p=top_p, num_samples=1)[:, :, 0] 
                        ### WATERMARKING END
                        end = time.time()
                        logger.info(f"Time needed for watermarking on scale {si}: {end-start:.2f}")
                        logger.info(f"Proportion 1 and 0: {torch.sum(idx_Bld, dim=1)/seq_len}")
                        if watermark.count_bit_flip: # Counts how many bits are actually flipped
                            idx_Bld_no_water = sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1)[:, :, 0]
                            function = lambda d, idx: torch.sum((~torch.eq(d[0][:, idx], d[1][:, idx])).to(torch.int32), dim=1).to(torch.int32).cpu().numpy().tolist()
                            if get_stat_data:
                                scale_bit_flips, token_bit_flips,bit_n_of_tokens_flips = analyse_bits_of_different_levels((idx_Bld.reshape(batch_size,-1), idx_Bld_no_water.reshape(batch_size,-1)), function, token_size, seq_len)
                                logger.info(f"Matches of watermark and no watermark {scale_bit_flips}/{idx_Bld.shape[-1]}") 
                    else: # no watermarking
                        idx_Bld = sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1)[:, :, 0]
                idx_Bld = idx_Bld.reshape(tmp_bs, tmp_seq_len, -1) 
                function = lambda d, idx: calc_entropy(d[:,idx,:])
                if get_stat_data:
                    scale_entropy, token_entropy,bit_n_of_tokens_entropy = analyse_bits_of_different_levels(logits_BlV, function, token_size, seq_len)
            else: # Use Var
            ...
            if vae_type != 0:
                ...
                if si < gt_leak:
                    ...
                    assert pn[0] == 1 #idx_Bld are the generated quantized bits
                ...
                # idx_Bld = torch.where(idx_Bld == 0, 1, 1) # sets all bits to 1
                idx_Bld_list.append(idx_Bld)
                
                # Transforms indices (0 or 1) to the corresponding code that is norm -1 or norm 1. Its further added to the feature (see summed codes)
                codes = vae.quantizer.lfq.indices_to_codes(idx_Bld, label_type='bit_label') # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
                if si != num_stages_minus_1:
                    tmp_summed_codes = summed_codes
                    summed_codes += F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                    if scales_injector and scales_injector.teacher_forcing and si in scales_injector.inject_scales:
                    # if scales_injector and scales_injector.teacher_forcing and si in scales_injector.inject_scales:
                        idx_Bld = scales_injector.inject_scales_bits[si]
                        if vae_type != 0:
                            assert returns_vemb
                            ...
                            # idx_Bld = torch.where(idx_Bld == 0, 1, 1) # sets all bits to 1
                        codes = vae.quantizer.lfq.indices_to_codes(idx_Bld, label_type='bit_label') # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
                        tmp_summed_codes += F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                    else:
                        tmp_summed_codes = summed_codes
                    last_stage = F.interpolate(tmp_summed_codes, size=vae_scale_schedule[si+1], mode=vae.quantizer.z_interplote_up) # [B, d, 1, h, w] or [B, d, 1, 2h, 2w]
            ...
            if vae_type != 0:
                if decode_per_scale:
                    upscaled_codes = F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                    img_residual_upscaled = vae.decode(upscaled_codes.squeeze(-3))
                    img_residual = vae.decode(codes.squeeze(-3))
                    img = vae.decode(summed_codes.squeeze(-3))
                    img_list.append(self.shape_image(img))
                    img_residual_list.append(self.shape_image(img_residual))
                    img_residual_upscaled_list.append(self.shape_image(img_residual_upscaled))
            else:
        ...
        if decode_per_scale:
            ...
            ret['img_residual_list'] = img_residual_list
            ret['img_residual_upscaled_list'] = img_residual_upscaled_list, 
        ret['summed_codes'] = summed_codes

        return ret, idx_Bld_list, img
    
    def shape_image(self, img):
        img = (img + 1) / 2
        img = img.permute(0, 2, 3, 1).mul_(255).to(torch.uint8).squeeze(0)
        return img
