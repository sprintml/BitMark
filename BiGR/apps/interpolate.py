...
def sample_func(model, bae, save, args, seed=0, image_size=256, num_classes=1000):
    ...
    labels1 = [args.cls1] # 355, 31, 245, 360, 207
    labels2 = [args.cls2]
    
    y1 = torch.tensor(labels1, device=device)
    y2 = torch.tensor(labels2, device=device)
    ...
    sample_list = []
    ratio_list = [0.0, 0.1, 0.2]
    ratio_list.extend(np.linspace(0.3,0.7,21).tolist())
    ratio_list.extend([0.8, 0.9, 1.0])
    
    for ratio_i in ratio_list:
        
        ratio = (ratio_i, 1.-ratio_i)
        print("Ratio: ", ratio)
        
        start_time = time()
        samples = model.generate_with_cfg(cond=y, max_new_tokens=latent_size ** 2, interpolate=ratio, cond_padding=args.cls_token_num, num_iter=args.num_sample_iter,
        ...
        save_i = save.rsplit('.', 1)[0] + f'_r{ratio_i:.2f}.png'
    ...
    save_i = save.rsplit('.', 1)[0] + '_all.png'
...
def main(args, args_ae):
    ...
    save_folder = os.path.join(args.save_path, f"{sample_index:03d}_{args.cls1}+{args.cls2}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interpolate_ratio", type=float, default=0.5)
    parser.add_argument("--cls1", type=int, required=True)
    parser.add_argument("--cls2", type=int, required=True)
