...
class BinaryQuantizer(nn.Module):
    ...
    def quantizer_two(self, x):
        x_det = (x > 0.5) * 1.0 
        x_ndet = torch.bernoulli(x)
        return x_det, x_ndet

    def forward_two(self, h):
        z = self.proj(h)

        z_det, z_b = self.quantizer_two(z)

        z_flow = z_b.detach() + z - z.detach()

        z_q = torch.einsum("b n h w, n d -> b d h w", z_flow, self.embed.weight)

        return z_q, z_b.detach(), z_det.detach()
...
class BinaryAutoEncoder(nn.Module):
    ...
    def encode(self, x):
        x = self.encoder(x)
        # quant, codebook_loss, quant_stats, binary = self.quantize(x, deterministic=self.deterministic)
        quant, binary, binary_det = self.quantize.forward_two(x)
        return quant, binary, binary_det
