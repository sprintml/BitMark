...
def postprocess(img_path, angle, image_size, save_img_dir=SAVE_IMAGE_DIR):
    ...
    # find the black edges in the images and crop them
    img = img.convert("RGB")
    img = img.crop(img.getbbox())
    img = img.resize((image_size, image_size), Image.ANTIALIAS)

    try:
        img.save(os.path.join(save_img_dir, filename))
        logger.info(f"Image after orientation angle correction has been saved here: {save_img_dir}/{filename}")
    except:
