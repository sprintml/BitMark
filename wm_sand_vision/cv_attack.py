...
class CLIPQualityOracle(QualityOracle):
    def __init__(self, quality_model_name_or_path="adams-story/HPSv2-hf") -> None:
        super().__init__()
        self.quality_oracle = CLIPModel.from_pretrained(quality_model_name_or_path).to("cuda:0")
    ...
    def judge(self, prompts, wtmk_images, candidate_images):
        with torch.no_grad():
            ...
            # move to device
            wtmk_inputs = {k: v.to("cuda:0") for k, v in wtmk_inputs.items()}
            candidate_inputs = {k: v.to("cuda:0") for k, v in candidate_inputs.items()}
...
class ImageRewardQualityOracle(QualityOracle):
    def __init__(self, quality_model_name_or_path="ImageReward-v1.0", device="cuda:0") -> None:
        super().__init__()
...
class Trainer():
    ...
    def attack(self, prompts, wtmk_images, img_ids, num_images_per_prompt=1, batch_idx=1, guidance_scale=5, mask_ratio=0.02, 
        ...
        for attack_step in tqdm(range(self.attack_steps)):
            ...
            if (attack_step+1) % save_interval == 0:
                for i, (image, img_id) in enumerate(zip(images, img_ids)):
                    save_path = os.path.join(save_folder, f"img{img_id}-step{attack_step+1}.png")
                    image.resize((1024, 1024)).save(save_path)
                    print("Saved image at", save_path)
            
        return images, valid_steps/batch_size

def main(scheme='wm_sand', sample_size=200, batch_size=50, attack_steps=200, mask_ratio=0.02,
    ...
    attack_cnt = 0
    base_path = '[WATERMARKED_IMAGE_DIR]'
    ...
    # dataset = WatermarkedImageDataset(img_folder, scheme=scheme, prompt_folder=prompt_folder, sample_size=sample_size)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=32)

    for batch_idx, batch in enumerate(dataloader):
        images, prompts, image_ids = batch['image'], batch['prompt'], batch['image_id']
        print(f'Attacking batch {batch_idx+1}/{len(dataloader)}: {len(prompts)} images')
        images = [Image.open(image) for image in images]
        for img_idx, (image, prompt, image_id) in enumerate(zip(images, prompts, image_ids)):
            image.save(os.path.join(f"{save_folder}", f"{scheme}", f"{image_id}-original.png"))
        attack_images, valid_steps = trainer.attack(prompts, images, image_ids, num_inference_steps=100, mask_ratio=mask_ratio,
                                                    save_folder=os.path.join(save_folder, f'{scheme}'), batch_idx=batch_idx)
        for prompt, attack_image, image, image_id in zip(prompts, attack_images, images, image_ids):
            attack_image.save(os.path.join(save_folder, scheme, f"{image_id}-attack.png"))
